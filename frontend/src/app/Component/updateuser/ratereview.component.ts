...
export class RatereviewComponent implements OnInit {
  ...
  rateNow(plat:any) {
    // if (this.visible) {
      //this.router.navigateByUrl('plats/ratingandreview/' + this.platId);
  ...
  addToCart(platId: any) {
    ...
    // if (this.visible) {
    //   this.platService.addToCart(this.platId).subscribe((response: any) => {
    //     this.data.changeMessage("count");
    //     console.log(response["obj"]);
    //     this.isAdded = response.obj;
    //     this._matSnackBar.open("plat added to cart", "ok", {
    //       duration: 1000,
    //     });
    //   });
    // } else {
